// Produces PP dijet-hadron correlations
// Original File:	Nick Elsey - 08.04.2016
// Adapted By:		Veronica Verkest - Nov.2016
...
int main ( int argc, const char** argv) {
  ...
  // ------------------------------
  // Pythia Defaults
  std::string 	executable    = "./bin/pythia_pp_correlation"; 	// placeholder
  std::string 	analysisType  = "ppdijet";
  bool requireDijets	= true;
  bool useEfficiency = false; // NO EFFICIENCY ON PYTHIA DATA!
  bool requireTrigger= false;
  double subJetPtMin   = 7.0; // subleading jet minimum pt
  double leadJetPtMin  = 14.0; // leading jet minimum pt
  double jetPtMax = 100.0;
  double jetRadius = 0.4;
  std::string outputDir = "tmp/";										// directory where everything will be saved
  std::string corrOutFile =  "pythiacorr.root";						// histograms will be saved here
  std::string treeOutFile = "pythiadijet.root";						// jets will be saved in a TTree here
  std::string inputFile = "Data/AddedGeantPythia/picoDst*.root";			// input file: can be .root, .txt, .list
  // std::string inputFile = "Data/AddedGeantPythia/picoDst_15_25_*.root";			// input file: can be .root, .txt, .list
